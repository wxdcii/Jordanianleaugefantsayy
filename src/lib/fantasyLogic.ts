...
export interface UserTransferState {
  savedFreeTransfers: number // Accumulated free transfers (no cap)
...
// Rule: Unused free transfers accumulate (+1 per gameweek, max 5), but penalties don't carry forward
export function calculateFreeTransfers(
  ...
  // GW3+: Add 1 free transfer to existing unused transfers, max 5
  const gameweekGap = toGameweek - fromGameweek;

  if (gameweekGap <= 1) {
    // Normal progression: add 1 free transfer, max 5
    const newTotal = Math.min(5, currentFreeTransfers + 1);
    console.log(`✅ Normal progression: ${currentFreeTransfers} + 1 = ${newTotal} (max 5)`);
    return newTotal;
  } else {
    // Gap in gameweeks: add 1 per missed gameweek, max 5
    const freeTransfersToAdd = gameweekGap;
    const newTotal = Math.min(5, currentFreeTransfers + freeTransfersToAdd);
    console.log(`✅ Gap progression: ${currentFreeTransfers} + ${freeTransfersToAdd} = ${newTotal} (max 5)`);
    return newTotal;
  }
}
...
export function processGameweekStart(
  ...
  const newState = {
    ...
    pointsDeductedThisWeek: 0, // Reset weekly deductions
...
export function testFreeTransferLogic() {
  ...
  console.log('GW3→GW4:', calculateFreeTransfers(3, 4, 2)); // Should be 3 (2+1)
  console.log('GW4→GW5:', calculateFreeTransfers(4, 5, 3)); // Should be 4 (3+1)
  console.log('GW5→GW6:', calculateFreeTransfers(5, 6, 4)); // Should be 5 (4+1, capped)
  console.log('GW6→GW7:', calculateFreeTransfers(6, 7, 5)); // Should be 5 (5+1, capped)
  ...
  console.log('GW4→GW5 (used 2):', calculateFreeTransfers(4, 5, 0)); // Should be 1 (0+1)

  // Test 3: Gameweek gaps
  console.log('\n📋 Test 3: Gameweek Gaps');
  console.log('GW3→GW6 (3 week gap):', calculateFreeTransfers(3, 6, 2)); // Should be 5 (2+3, capped)
  console.log('GW5→GW10 (5 week gap):', calculateFreeTransfers(5, 10, 1)); // Should be 5 (1+5, capped)
  ...
  console.log('2 transfers, 3 FT:', calculateTransferCost(2, 3, 5, false, false));
