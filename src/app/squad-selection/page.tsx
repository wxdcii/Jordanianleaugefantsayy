...
const PlayerKit = ({
  ...
  return (
    <div className={`${sizeClasses[size]} ${className} relative flex-shrink-0`}>
      {/* Jersey body with gradient for 3D effect */}
      <div
...
>
  <img
    ...
    style={{
      boxShadow: '0 4px 4px rgba(0,0,0,0.10)', // Stronger shadow
      borderRadius: '100%'
    }}
  />
</div>
...
export default function SquadSelectionPage() {
  ...
  useEffect(() => {
    ...
    loadUserData();
  }, [user, currentGameweek, openGameweek]);
  ...
  const handleSaveTeam = async () => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('تشكيلة غير صحيحة')) {
      ...
      >
        ...
        <div className="relative w-16 h-20 mx-auto mb-2">
          <PlayerKit
            ...
            size="large"
            className="w-full h-full shadow-lg border-2 border-white rounded-lg"
          />
        </div>
            ...
            {(() => {
              ...
              <div className={`text-white p-4 rounded-lg ${chipsUsed.benchBoost.isActive ? 'bg-green-600 border-2 border-green-400' : 'bg-gray-800'}`}>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <div className="text-center">
                    <div className="text-xs text-gray-400 mb-1">GK</div>
                    {(() => {
                  ...
                  {[1, 2, 3].map((spotIndex) => {
                    ...
                    return (
                      <div key={`bench-${spotIndex}`} className="text-center">
                        <div className="text-xs text-gray-400 mb-1">
                          {player ? player.position : 'OUT'}
                        </div>
                        {player ? (
                          <div 
                            className={`cursor-pointer hover:scale-105 transition-transform ${
                          ...
                          >
                            <div className={`rounded-full w-12 h-12 flex items-center justify-center text-xs font-bold mx-auto mb-1 ${
